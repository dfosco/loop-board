...
[ -e "$BOARD" ] || { echo "loop.sh: board directory not found: $BOARD (set BOARD=<path> to override)" >&2; exit 1; }
[ -d "$BOARD" ] || { echo "loop.sh: board path is not a directory: $BOARD" >&2; exit 1; }
